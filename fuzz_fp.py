...
def main():
    ...
    while True:
        ...
        env = os.environ.copy()
        prediff = forksubprocess(gen, rng, case_path, case_id, env,'init')
        if prediff == None:
            endhandle(progress, 0, case_path)
            continue
        if prediff==np.inf or prediff!=prediff:
            progress.update()
            continue
        # prepare runmodule
        ...
        while not q.empty():
            ...
            if (lastdiff > seedinitdiff):
                q.put(lastinputarr)
                cr_imageseed, noneflag = cross(main_fn,lastinputarr,usedseed,gmod1,gmod5)
                if(noneflag!=None):
                    q.put(cr_imageseed)
